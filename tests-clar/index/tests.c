...
#define HEY_THERE_OID "a8233120f6ad708f843d861ce2b7228ec4e3dec6"
...
void test_index_tests__add(void)
{
	 ...
	 */
	cl_git_pass(git_oid_fromstr(&id1, HEY_THERE_OID));
...
void test_index_tests__add_in_subdirectory(void)
{
	git_repository *repo;
	git_index *index;
	const git_index_entry *entry;
	...
	cl_git_pass(git_repository_init(&repo, "myrepo", 0));
	cl_git_pass(git_repository_index(&index, repo));
	cl_assert_equal_sz(0, git_index_entrycount(index));

	/* Create a new file in a subdirectory */
	cl_git_pass(git_futils_mkpath2file("myrepo/subdir/test.txt", 0777));
	cl_git_mkfile("myrepo/subdir/test.txt", "hey there\n");

	/* Add the new file to the index */
	cl_git_pass(git_index_add_bypath(index, "subdir/test.txt"));

	cl_assert_equal_sz(1, git_index_entrycount(index));
	cl_assert((entry = git_index_get_byindex(index, 0)) != NULL);
	cl_assert(git_oid_streq(&entry->oid, HEY_THERE_OID) == 0);

	git_index_free(index);
	git_repository_free(repo);
}

void test_index_tests__add_all_in_subdirectory(void)
{
	git_repository *repo;
	git_index *index;
	const git_index_entry *entry;
	...
	paths.strings = git__malloc(sizeof(char *));
	paths.strings[0] = "*";
	paths.count = 1;

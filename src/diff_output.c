	...
	git_diff_delta *delta)
{
	...
	if (ctxt->opts &&
		(ctxt->opts->flags & GIT_DIFF_FORCE_TEXT) != 0)
	{
	...
	diff_context *ctxt, git_diff_patch *patch)
{
	...
	error = diff_delta_is_binary_by_attr(ctxt, delta);
...
cleanup:
	...
	if (!error) {
		...
		if (delta->binary != 1 &&
			...
			!git_oid_equal(&delta->old_file.oid, &delta->new_file.old))
...
static int diff_patch_cb(void *priv, mmbuffer_t *bufs, int len)
{
	...
	if (len == 2 || len == 3) {
		...
		if (ctxt->line_cb != NULL &&
			ctxt->line_cb(ctxt->cb_data, patch->delta, &ctxt->cb_range,
	...
	size_t content_len)
{
	...
	assert(patch->hunk_size > 0);
	assert(patch->hunks != NULL);

	hunk = patch->hunks[patch->hunk_size - 1];
	...
	last = (patch->lines_size > 0) ? patch->lines[patch->lines_size - 1] : NULL;
	...
	git_diff_data_fn line_cb)
{
	...
	diff_context_init(
		...
		cb_data, file_cb, hunk_cb, line_cb);

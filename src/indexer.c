...
static int index_path(git_path* path, git_indexer *idx)
{
	...
	*ptr = '\0';
	...
	error = git__path_strcat(path, prefix);

	if (!error) {
		git_oid_fmt(oid, &idx->hash);
		oid[GIT_OID_HEXSZ] = '\0';
		error = git__path_strcat(path, oid);
	}

	if (!error)
		git__path_strcat(path, suffix);
...
int git_indexer_write(git_indexer *idx)
{
	...
	if ((error = git__path_strcpy(&filename, idx->pack->pack_name)) < GIT_SUCCESS)
		return error;
	filename.data[strlen(filename.data) - strlen("pack")] = '\0';
	if ((error = git__path_strcat(&filename, "idx")) < GIT_SUCCESS)
		return error;
